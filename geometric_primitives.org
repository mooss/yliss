...
class vertices {
  public:
    ...
    vertices(
        const Container<Inner>& content, const std::vector<std::size_t>& layout
    ):
        count_(content.size() / lit::sum(layout))
    {
        if(layout.size() == 0)
            throw std::runtime_error("Empty vertex layouts are illegal.");
        details::assert_multiple(content.size(), lit::sum(layout), "vertices for the given layout");
...
for(std::size_t i = 0; i < layout.size(); ++i) {
    glVertexAttribPointer(
        i, layout[i], details::glenum<Inner>::v, GL_FALSE,
        layout[i] * sizeof(Inner), (void*) offset
    );
    offset += sizeof(Inner) * layout[i];
...
template<typename T>
vertices<std::vector, T> vertices_vector(const std::vector<T>& content, const std::vector<std::size_t>& layout){
    return vertices(content, layout);
}
#+end_src
...
int main(){
    ...
    VAO triangle_vao{};
    auto triangle = vertices_vector<GLfloat>({
        -0.5, -0.5, 0.0,
         0.5, -0.5, 0.0,
         0.0,  0.5, 0.0
    }, {3});
...
int main(){
    ...
    shader_program basic_shader{vertex_shader(vs_str), fragment_shader(fs_str)};
    VAO rectangle_vao{};
    auto rectangle_vertices = vertices_vector<GLfloat>({
...
This example is coming from the Shaders section (https://learnopengl.com/Getting-started/Shaders).
...
#+begin_src cpp :noweb no-export :eval no-export :flags -std=c++20 -I include -ldl -lGL -lglfw src/glad.c
<<include(":noweb lazy_window VAO shader_program vertices_vector uniform4f :cpp cmath")>>
