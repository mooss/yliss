...
See below the textual equivalent of figure [[montage_squa_degree_0-3]]:
#+BEGIN_SRC python
<<Textual squarified net>>
between = ' ' * 25
for deg0, deg1 in zip(SquareNet(3, [0]).repr(), SquareNet(3, [0, 1]).repr()):
    print(deg0 + between + deg1)
print('           degree = 0           ' + between + '            degree = 1')
print()
print()
for deg0, deg1 in zip(SquareNet(3, [0, 1, 2]).repr(), SquareNet(3, [0, 1, 2, 3]).repr()):
    print(deg0 + between + deg1)
...
There are two things to address right now about this representation.

The first is that it bears an unfortunate ressemblance with a swastika.
...
The second thing to address is in fact the source of the first ; it looks this way because the redundant points have been removed.
The most striking example of a redundant point are the corners, all the corners are in fact one and the same, so only the top-right one is displayed.
The corners and the redundant points in general will be discussed in detail in the sibling section [[Coordinate system]].
...
def raw(self, appearance=None):
    ...
    if appearance is None:
        topbottom = Quarter.CORNER + Quarter.HORIZONTAL * (length - 2) + Quarter.CORNER
        emptyline = Quarter.VERTICAL + Quarter.EMPTY * (length - 2) + Quarter.VERTICAL
    ...
    if appearance == 0:
        top = Quarter.CORNER + Quarter.HORIZONTAL * (length - 2) + (Quarter.POINT if self.quarter == 0 or self.display_redundant else Quarter.CORNER)
        bottom = (Quarter.POINT if self.quarter == 0 else Quarter.CORNER) + Quarter.HORIZONTAL * (length - 2) + Quarter.POINT
        emptyline = Quarter.VERTICAL + Quarter.EMPTY * (length - 2) + Quarter.VERTICAL
    ...
    # Build lines.
    right_vertical = Quarter.POINT if self.display_redundant else Quarter.VERTICAL
    topbottom = Quarter.CORNER + small_horizontal + big_horizontal.join([Quarter.POINT] * npoints_even) + small_horizontal + Quarter.CORNER
    emptylines = [Quarter.VERTICAL + Quarter.EMPTY * (length - 2) + Quarter.VERTICAL] * nspaces_odd
    oddline = Quarter.VERTICAL + small_empty + small_empty.join([Quarter.POINT] * npoints_odd) + small_empty + right_vertical
    evenline = Quarter.VERTICAL + small_empty + big_empty.join([Quarter.POINT] * npoints_even) + small_empty + Quarter.VERTICAL
...
Most of the work has already been done in =Quarter= so the arguments are the same, except for =quarter= which becomes =quarters=, illustrating that what's left to be done is to assemble the four quarters together.
...
class SquareNet:
    def __init__(
            self, degree, appearance=None,
            quarters={0, 1, 2, 3}, display_redundant=False
    ):
        self.degree = degree
        self.appearance = appearance
...
#+BEGIN_SRC python
def repr(self):
    top = cat_quarter_horizontally(self.quarters[1].repr(), self.quarters[0].repr())
    bottom = cat_quarter_horizontally(self.quarters[2].repr(), self.quarters[3].repr())
    return cat_quarter_vertically(top, bottom)
#+END_SRC
...
<<Textual squarified net>>
degree=2
print('Right quarters')
print(SquareNet(degree, '*', quarters={0, 3}))
print()
print('Bottom quarters')
print(SquareNet(degree, '*', quarters={2, 3}))
print()
print('All quarters without the last points')
print(SquareNet(degree, list(range(degree))))
print()
print('All quarters, only the last points')
print(SquareNet(degree, degree))
#+END_SRC
...
All quarters without the last points
+-------x---x---x
|       |       |
x   x   x   x   |
|       |       |
...
All quarters, only the last points
+-------+-x---x-+
x x x x x x x x |
| x   x | x   x |
x x x x x x x x |
