...
print()
print('All quarters, only the last points, with redundant characters')
print(SquareNet(degree, degree, display_redundant=True))
#+END_SRC
...
All quarters, only the last points, with redundant characters
+-v---v-+-x---x-+
x x x x x x x x v
| x   x | x   x |
x x x x x x x x v
