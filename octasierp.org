...
See below the textual equivalent of figure [[montage_squa_degree_0-3]] [fn::=x= are points, =-= are horizontal lines, =|= are vertical lines and =+= are corners. The representations have been scale to all appear at the same size, that is normally not the case.]:
#+BEGIN_SRC python
<<Textual squarified net>>
between = ' ' * 25
def construct(degree):
    return SquareNet(3, list(range(degree+1)), display_redundant=True).repr()
