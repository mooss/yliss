...
#+property: header-args:python :noweb no-export :results output :wrap "src text :minipage"
...
class Triangle(object):
    def __init__(self, degree, initial_width, points_geometry=None, lines_geometry=None):
        self.degree = degree
        self.initial_width = initial_width
        if points_geometry is None:
            self.points_geometry = Geometry(degree)
        else:
            self.points_geometry = points_geometry
        if lines_geometry is None:
...
The evolution of an =Octanet= from an octahedron to a quasi-squaregrid is tracked through the =progress= parameter of =make_surface=:
...
#+name: Octanet/__init__
#+begin_src jupyter-python
def __init__(
        self, scale, degree=0, points_geometry=None, lines_geometry=None, surface_parameters={}
):
    self.degree = degree
    ...
    triangle = Triangle(degree, stroke_width, points_geometry=points_geometry, lines_geometry=lines_geometry)
...
#+begin_src python :tangle script/animations.py :results silent :eval no
<<Octanet>>

degree = 3
no_display = Geometry(0, color_palette=[colors.air])
...
yliss_logo = Octanet(
    scale=15, degree=degree,
    lines_geometry=no_display,
    points_geometry=points_geometry,
    surface_parameters={'bg_color': colors.black}
)
#+end_src
...
yliss_logo.make_surface().write_to_png('images/octahedron_net.png')
yliss_logo.make_surface(1).write_to_png('images/octahedron_net_as_square.png')
#+end_src
...
Finally, using the same definition, an animation from octahedron to quasi-squarenet is stitched together:
...
def generate_frame(time):
    return yliss_logo.make_surface(time / animation_duration).get_npimage()
