...
def raw(self, appearance=None):
    ...
    if appearance == 0:
        top = self.CORNER + self.HORIZONTAL * (length - 2) + (self.POINT if self.quarter == 0 or self.display_redundant else self.CORNER)
    ...
    # Build lines.
    right_vertical = self.POINT if self.display_redundant else self.VERTICAL
    ...
    oddline = self.VERTICAL + small_empty + small_empty.join([self.POINT] * npoints_odd) + small_empty + right_vertical
