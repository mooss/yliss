...
Here is the updated example from the previous session reflecting the changes we just made, with =v= characters representing a redundant point[fn::The canonical counterparts of those points are still represented with an =x=.]:
#+begin_src python :exports results
...
#+begin_src bash :tangle script/illustration_0_through_3.sh :shebang "#!/usr/bin/env bash"
