...
class octanet(object):
    ...
    def triangles(self, a, b, c, degree=0):
        stroke_width = self.stroke_width * (- degree / (1.5 * self.degree) + 1)
        ...
        return gz.Group([
            main_triangle,
            self.triangles(a, ab, ac, degree + 1),
            self.triangles(b, ab, bc, degree + 1),
            self.triangles(c, ac, bc, degree + 1),
            self.triangles(ab, ac, bc, degree + 1) # sierpinsky if absent
        ] + points)
...
scale = 5
animation = octanet(
    100 * scale, 10 * scale, 30 * scale, 2 * scale,
    degree=2
)
