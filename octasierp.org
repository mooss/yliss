...
This notably means that given the coordinates of one point inside the quarter, we need to find the degree at which it first appeared (i.e. its degree of appearance) before accessing it.
This is linked with what I call the binary degree of a number.
...
Here is a breakdown of the arguments accepted by Quarter's init:
 ...
 - =display_point= contains points representations for each degree.
   Representations will be cycled through if the list is too short.
...
To make life easier, the string =*= is a shortcut for all the degrees of appearance possible:
#+begin_src python :noweb-ref "Textual quarter init"
if self.appearance == '*':
...
if len(self.display_points) < max_appearance + 1:
    tmp = [] + self.display_points
    for representation in itertools.cycle(tmp):
        self.display_points.append(representation)
        if len(self.display_points) >= max_appearance + 1:
            break
#+end_src
