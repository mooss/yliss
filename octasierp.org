...
For example, at degree 2 the points $A = (4, -3)$ and $B = (4, 3)$ are the same, as well as the points $C = (2, -1)$ and $D = (2, 1)$:
#+BEGIN_SRC python
<<Textual squarified net>>
repre = SquareNet(2, [0, 1, 2], display_redundant='x').repr()
first = repre[0]
repre[0] = first[:2] + 'A' + first[3:-3] + 'B' + first[-2:]
repre[2] = repre[2][:-1] + 'C'
repre[6] = repre[2][:-1] + 'D'
print('\n'.join(repre))
#+END_SRC
...
CAN_COLLIDE = {
    ...
    CORNER, CENTRAL_EMPTY
}
IS_POINT = {POINT, COLLISION, REDUNDANT, CENTRAL_POINT}
#+END_SRC
...
def raw(self, appearance=None):
    ...
    if appearance is None:
        topbottom = Quarter.CORNER + Quarter.HORIZONTAL * (length - 2)\
            + Quarter.CORNER
    ...
    if appearance == 0:
        if self.quarter == 0:
            topright = Quarter.POINT
        elif self.display_redundant:
            topright = self.redundant
        else:
            topright = Quarter.CORNER
        top = Quarter.CORNER + Quarter.HORIZONTAL * (length - 2) + topright
        bottom = (Quarter.POINT if self.quarter == 0 else Quarter.CORNER)\
    ...
    topbottom = Quarter.CORNER + small_horizontal\
    ...
    fulllines = [topbottom] + ([oddline, evenline]) * nlines_even\
        + [oddline, topbottom]
    def joinlines():
        it = iter(fulllines)
        yield next(it)
