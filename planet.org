...
#+property: header-args:cpp :flags -std=c++20 -Wall -I include src/glad.c -ldl -lGL -lglfw :eval never :main no :noweb no-export
...
#+begin_src cpp :noweb-ref octaform/impl :minipage
template<class HeightGetter, template <std::size_t> class... SubVertices>
...
#+end_src
#+depends:octaform :noweb pointless_octasierp meta::list meta::cat meta::steal dynasty :cpp vector span utility

*** Sub-vertices

The methods below will be used by the different sub-vertices to access to the relevant parts of vertices.
...
To summarise the role of =octaform=, here is a short list of what it will make available to the octasierp policies:
 ...
 - A pointer and a span to the desired offseted vertex (via the =offset_ptr= and =offset_span= methods).
...
** Sub-vertices specification

Sub-vertices can be defined via the template arguments of an octaform.
The idea is to be able to reuse the octaform class using completely different data layouts.

*** Instantiation

Instantiation of sub-vertices occurs in the octaform itself.
It consists of transforming the sub-vertices specification into a dynastic type with an octaform founder.
That is to say, a type who will inherit from octaform as well as from all sub-vertices.
It will therefore have access to all octaform methods and to all the methods provided by the sub-vertices.

The most important parts of this code are:
 - =impl= :: The actual type of the octaform.
 - =vertex_size= :: The total size of a vertex, computed from the sizes of the individual sub-vertices.

#+begin_src cpp :noweb-ref octaform/public :minipage
template<std::size_t Offset, template <std::size_t> class... SubVertices2>
struct instantiate;

template<std::size_t Offset, template <std::size_t> class SubVertex>
struct instantiate<Offset, SubVertex> {
    using type = SubVertex<Offset>;
    using list = meta::list<type>;
    static constexpr std::size_t vertex_size = Offset + type::size;
};

template<
    std::size_t Offset,
    template <std::size_t> class Head,
    template <std::size_t> class... Tail
> struct instantiate<Offset, Head, Tail...> {
    using up_ = instantiate<Offset + Head<0>::size, Tail...>;
    using type = Head<Offset>;
    using list = meta::cat<type, typename up_::list>;
};

using instantiated = instantiate<0, SubVertices...>;
using impl = meta::steal<dynasty, meta::cat<octaform, typename instantiated::list>>;
static constexpr std::size_t vertex_size = instantiated::vertex_size;
#+end_src

*** Color and position

The color and position sub-vertices have the same size, they only differ in the name of their methods.
...
struct rgb {
    ...
    static constexpr std::size_t rgb_offset = Offset;
    template<class Octaform> struct heir: Octaform {
        auto rgb_ptr(std::size_t index) {return offset_ptr<Offset>(index);}
        auto rgb_span(std::size_t index) {return offset_span<size, Offset>(index);}
    };
};
}
#+end_src

#+name: subvertex/pos_3d
#+begin_src cpp :minipage
namespace subvertex {
template<std::size_t Offset>
struct pos_3d {
    static constexpr std::size_t size = 3;
    template<class Octaform> struct heir: Octaform {
        static constexpr std::size_t position_offset = Offset;
        auto position_ptr(std::size_t index) {return offset_ptr<Offset>(index);}
        auto position_span(std::size_t index) {return offset_span<size, Offset>(index);}
    };
};
}
#+end_src
...
auto grow_octasierp(
    ...
    using namespace subvertex;
    using actual_octaform = typename octaform<decltype(elevation(base)), pos_3d, rgb>::impl;
    actual_octaform ofo({base, elevation(base)});
...
#+end_src
#+depends:grow_octasierp :noweb octaform subvertex/pos_3d subvertex/rgb :cpp utility
...
struct rm5cy_bnws_coloration {
    ...
    void fill_colors(Octaform& ofo) {
        ...
        for(std::size_t i = 0; i < end; ++i)
            gradient.fill(ofo.rgb_span(i), i, end);

        auto pole = ofo.rgb_span(0); pole[0] = 0; pole[1] = 0; pole[2] = 0;
        pole      = ofo.rgb_span(1); pole[0] = 1; pole[1] = 1; pole[2] = 1;
    }
};
#+end_src
...
void height_coloration_octasierp(
    ...
    for(std::size_t i = 0; i < end_index; ++i) {
        ...
        if(height <= ofo.stats.sea_height)
            below.fill(ofo.rgb_span(i), height);
        else
            above.fill(ofo.rgb_span(i), height);
    }
}
#+end_src
...
#+begin_src cpp :eval no-export :exports both
<<include(":noweb grow_octasierp midpoint_elevation height_coloration gfg_color_scheme bumpy_surface_waters spherical_repartition fill_octasierp_normals uniform_3f render_octasierp prerender_phong drawspin")>>
