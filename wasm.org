...
#+name: cc-ll
#+begin_src sh :var block="" language="c++" :results output :wrap "src llvm :exports code" :eval no-export
function die() {
    echo "$@"
    exit
}
...
#+begin_src cpp :name bad-add-function
int add(int a, int b) {
    return a * a + b;
}
#+end_src
